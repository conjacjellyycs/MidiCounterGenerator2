...
namespace Midi_Counter_Generator
{
    ...
    class mainpart
    {
        ...
        public void read()
        {
            ...
            Console.Write("Input video filename (Default: MIDIname+.counter.mp4): ");
            fileout = (sss = Console.ReadLine()) == "" ? filein + ".counter.mp4" : sss;
            if (fileout[0] == '\"')
            {
                string file1n = "";
            ...
            delay *= fps;
            Console.Write("Do you want to use fast render? (Input 1 if the answer is YES, 0 otherwise, default: 0)");
        ...
        struct Patterns
        {
            ...
            public void draw(long nc, long an, double bp, double tm, long np, long po, long ti, long at, int id, string tmpfol)
            {
                using (var font = new Font(fon, fontsz))
                {
                    ...
                    {
                        gfx.FillRectangle(Brushes.Black, 0, 0, image.Width, image.Height);
        ...
        void OriginalRender()
        {
            ...
            {
                Console.WriteLine("Positioned track {0}/{1}", i + 1, trkcnt);
                for (int j = 0; j < 4; j++)
                {
                    toint(inp.ReadByte());
                    cntpos++;
            ...
            while (faq > 0)
            {
                ...
                pts.draw(notecnt, noteall, 60000000.0 / resol / ((pairli)bpm[bpmptr]).y, 1.0 * (tmdf - delay) / fps, notecnt - history[tmdf % fps], poly, Convert.ToInt64(tmm), alltic, tmdf, tmpf);
        ...
        void FastRender()
        {
            Console.WriteLine("Fast Render is not supported yet. Falling back to original render.");
            OriginalRender();
        }
        public void Render()
        {
